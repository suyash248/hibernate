...
public class Main {
	...
	public static List<Student> fetchRecords(Session session){
	...
	public static void deleteRecords(Session session){
		List<Student> studentsList = fetchRecords(session);
		for(Student student : studentsList){
	...
	public static void insertRecords(Session session){
       ...
       // Using CascadeType.PERSIST so no need to save/persist any Address object explicitly.
       session.persist(s1);
	}
}
